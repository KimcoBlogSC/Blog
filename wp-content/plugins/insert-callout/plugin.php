...
{
    global $alignment, $starttag, $titletag, $closetag, $endtag, $closealign;
	...
	$alignpos = strpos($the_content, $alignment, 0);  	// this could be false
	$alignposclose = strpos($the_content, $closealign, $alignpos);
	...
	if ($closepos !== false)
	{
		...
		if (($alignpos !== false) && ($alignpos < $endpos))
		{
			$alignlen = $alignposclose - ($alignpos + strlen($alignment));
			$the_align = ' ' . substr($the_content, $alignpos + strlen($alignment), $alignlen);
		}
		else
		{
			$the_align = ' default';
		...
		$the_callout = '<div class="callout' . $the_align . '"><div class="callout_title" style="'.$callout_title_style.
						'">'.$the_title.'</div><div class="callout_body" style="'.$callout_body_style.
						'">'.$the_body.'</div></div>';
...
function metabox ($post)
{
	...
	echo '<p>Add a callout to your post using the following syntax: </p>' .
		 '<p>[callout align=full | title=Callout Title]The body of your callout goes here.[/callout]</p>' .
		 '<p>Change the orientation of the callout by modifying the "align" property. Orientation options include: full, left and right.</p>';
}
